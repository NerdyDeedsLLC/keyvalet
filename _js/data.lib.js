...
const parentSelector = function (elem, selector, closestReturn=true) {
    ...
    for ( ; elem && elem !== document; elem = elem.parentNode ) {
        if ( elem.matches( selector ) ){ matchedElements.push(elem); }
    }
...
EOF`,
        blob = new Blob([scriptText], { type: 'text/plain' }),
...
const contactAuthor = (diagID='PATCHA') => {
    ...
    const resolve = (diagID, result, data) => {
        d.getElementById(diagID).style.display='none';
        h.className = h.className.replace(" dialog-visible", "");
        var event = new CustomEvent('sendEmail', {detail: {dialogID:diagID, dialogValue:result, data:data}, bubbles:false});
        this.dispatchEvent(event);
    };
    okButton.onclick     = () => { resolve('RemoveDialog', true, {sourceObj, diagID, instanceCt, keyID}); };
    cancelButton.onclick = () => { resolve('RemoveDialog', false, {sourceObj, diagID, instanceCt, keyID}); };
...
const resolveDialog = (e, detail=e.detail, dialog=detail.dialogID, value=detail.dialogValue, data=detail.data) => {
    ...
    switch(dialog){
        case 'RemoveDialog': // Removes a Key Binding
            ...
            if(value && bindingToRemove){ setTimeout(function(){ bindingToRemove.remove(); processRules(); }, 750); }
        break;
        case 'sendEmail':
...
const instructNav = (operator='+') =>{
    ...
    var activePanel = 0;
    allPanels.forEach(p => {
        if(w.getComputedStyle(p).display !== 'none'){ activePanel = p.className.replace(/\s*bubble-set\s*/, ''); }
...
var reticle = {
    ...
    place: function(x=0, y=0) {                                                         // FN: place(x,y)       Drops the reticle at the X/Y coordinates specified. 
    ...
    color: function(clr) {                                                              // FN: color(clr)       Sets the reticle's and its text's color to the specified string
    ...
    orient: function(objQS, rPos='tl', offsetWidth=0, offsetHeight=0)  {
        try{
            ...
            let osT = objBCR.y + offsetHeight,                       // Note we add the user-set offsets to our bases
                osL = objBCR.x + offsetWidth;                        // so they carry though to the other options.
            if(/m/i.test(rPos))      { osT += (objBCR.height / 2); } // Adjust vertically for 'm'iddle (top + height/2)
            if(/b/i.test(rPos))      { osT += objBCR.height; }       // Adjust vertically for 'b'ottom (top + height)
            if(/c/i.test(rPos))      { osL += (objBCR.width / 2); }  // Adjust horizontally for 'c'enter (left + width/2)
            if(/r/i.test(rPos))      { osL += objBCR.width; }        // Adjust horizontally for 'r'ight (left + width)
...
var calloutBubbles = {
    ...
    shiftBubble: function(bbl, bblArrow, bblDimens) {
      console.log('shiftBubble', 'bbl:', bbl, 'bblArrow:', bblArrow, 'bblDimens:', bblDimens)
        let shift = {offsetTop:0, offsetLeft:0};
